...
class Revo2Node(Node):
    def __init__(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        self.declare_parameter("speeds", [1000, 1000, 1000, 1000, 1000, 1000])
        ...
        self._sub_trig_right = self.create_subscription(
            Float64,
            "vr_trig_right",
        ...
        # button A(right hand) subscriber
        self._sub_button_a = self.create_subscription(
            Bool,
            "vr_a_button",
        ...
        self._sub_trigger_left = self.create_subscription(
            Float64,
            "vr_trig_left",
        ...
        # button X(left hand) subscriber
        self._sub_button_x = self.create_subscription(
            Bool,
            "vr_x_button",
            self._button_x_callback,
            10,  # QoS queue size
        )
    ...
    def _lh_trig_callback(self, msg: Float64):
        if msg.data > 0.3:
            self._left_trigger = 1
        else:
            self._left_trigger = 0
            
    def _button_x_callback(self, msg: Bool):
        if msg.data:
            self._button_x = 1
        else:
            self._button_x = 0
    ...
    async def _async_setup_and_run(self):
        ...
        try:
            await self._client.set_finger_positions(self._right_hand_id, [350, 400, 0, 0, 0, 0])
            await self._client.set_finger_positions(self._left_hand_id, [350, 400, 0, 0, 0, 0])
            await asyncio.sleep(1.0)
            while not self._shutdown_event.is_set():
                # await self._client.set_serialport_baudrate(self._left_hand_id, libstark.Baudrate.Baud1Mbps)
                try:
                    right_posi = await self._client.get_finger_positions(self._right_hand_id)
                    left_posi = await self._client.get_finger_positions(self._left_hand_id)
                    right_middle_posi = right_posi[3]
                    right_index_posi = right_posi[2]
                    right_thumb_posi = right_posi[1]
                    left_middle_posi = left_posi[3]
                    left_thumb_posi = left_posi[1]
                    left_index_posi = left_posi[2]
                    if self._right_trigger == 0 and self._button_a == 0 and right_thumb_posi > 500 and right_index_posi < 300:
                        await self._client.set_finger_positions(self._right_hand_id, [350, 400, 0, 1000, 1000, 1000])
                    elif self._right_trigger == 0 and self._button_a == 0 and right_thumb_posi > 500 and right_index_posi > 300:
                        await self._client.set_finger_positions(self._right_hand_id, [350, 400, 1000, 1000, 1000, 1000])
                    elif self._right_trigger == 0 and self._button_a == 0 and right_thumb_posi < 500:
                        await self._client.set_finger_positions(self._right_hand_id, [350, 400, 0, 0, 0, 0])
                    elif self._right_trigger == 1 and self._button_a == 0 and right_middle_posi < 500:
                        await self._client.set_finger_positions(self._right_hand_id, [350, 400, 0, 1000, 1000, 1000])
                    elif self._right_trigger == 1 and self._button_a == 0 and right_middle_posi > 500:
                        await self._client.set_finger_positions(self._right_hand_id, [600, 750, 0, 1000, 1000, 1000])
                    elif self._right_trigger == 0 and self._button_a == 1 and right_middle_posi < 750:
                        await self._client.set_finger_positions(self._right_hand_id, [350, 400, 1000, 1000, 1000, 1000])
                    elif self._right_trigger == 0 and self._button_a == 1 and right_middle_posi > 750:
                        await self._client.set_finger_positions(self._right_hand_id, [600, 750, 1000, 1000, 1000, 1000])
                    
                    if self._left_trigger == 0 and self._button_x == 0 and left_thumb_posi > 500 and left_index_posi < 300:
                        await self._client.set_finger_positions(self._left_hand_id, [350, 400, 0, 1000, 1000, 1000])
                    elif self._left_trigger == 0 and self._button_x == 0 and left_thumb_posi > 500 and left_index_posi > 300:
                        await self._client.set_finger_positions(self._left_hand_id, [350, 400, 1000, 1000, 1000, 1000])
                    elif self._left_trigger == 0 and self._button_x == 0 and left_thumb_posi < 500:
                        await self._client.set_finger_positions(self._left_hand_id, [350, 400, 0, 0, 0, 0])
                    elif self._left_trigger == 1 and self._button_x == 0 and left_middle_posi < 500:
                        await self._client.set_finger_positions(self._left_hand_id, [350, 400, 0, 1000, 1000, 1000])
                    elif self._left_trigger == 1 and self._button_x == 0 and left_middle_posi > 500:
                        await self._client.set_finger_positions(self._left_hand_id, [600, 750, 0, 1000, 1000, 1000])
                    elif self._left_trigger == 0 and self._button_x == 1 and left_middle_posi < 750:
                        await self._client.set_finger_positions(self._left_hand_id, [350, 400, 1000, 1000, 1000, 1000])
                    elif self._left_trigger == 0 and self._button_x == 1 and left_middle_posi > 750:
                        await self._client.set_finger_positions(self._left_hand_id, [600, 750, 1000, 1000, 1000, 1000])
