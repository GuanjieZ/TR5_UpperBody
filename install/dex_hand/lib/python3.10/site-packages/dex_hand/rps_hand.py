...
def _baud_from_param(name: str) -> "libstark.Baudrate":
    try:
        ...
        return libstark.Baudrate.Baud1Mbps
...
class Revo2Node(Node):
    def __init__(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        self.declare_parameter("speeds", [750, 400, 750, 750, 750, 750])
        self.declare_parameter("rock", [0, 0, 1000, 1000, 1000, 1000])
        self.declare_parameter("paper", [0, 0, 0, 0, 0, 0])
        self.declare_parameter("scissors", [500, 800, 0, 0, 1000, 1000])
        ...
        self._pos_rock: List[int] = list(self.get_parameter("rock").get_parameter_value().integer_array_value)
        self._pos_paper: List[int] = list(self.get_parameter("paper").get_parameter_value().integer_array_value)
        self._pos_scissors: List[int] = list(self.get_parameter("scissors").get_parameter_value().integer_array_value)

        self._client: Optional["libstark.PyDeviceContext"] = None
        self._shutdown_event = asyncio.Event()
        self._run_task: Optional[asyncio.Task] = None
        
        self._gesture = self._pos_paper
    ...
    def _trig_callback(self, msg: String):
        if msg.data == "paper":
            self._gesture = self._pos_scissors
        elif msg.data == "scissors":
            self._gesture = self._pos_rock
        else:
            self._gesture = self._pos_paper
    ...
    async def _async_setup_and_run(self):
        ...
        try:
            await self._client.set_finger_positions(self._slave_id, self._pos_paper)
            await asyncio.sleep(1.0)
            while not self._shutdown_event.is_set():
                try:
                    # await self._client.set_finger_currents(self._slave_id, self._current)
                    await self._client.set_finger_positions(self._slave_id, self._gesture)
                except Exception as e:
