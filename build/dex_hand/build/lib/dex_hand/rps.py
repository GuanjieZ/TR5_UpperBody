...
class Revo2Node(Node):
    def __init__(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        self.declare_parameter("rock", [0, 0, 0, 0, 0, 0])
        self.declare_parameter("paper", [0, 0, 0, 0, 0, 0])
        self.declare_parameter("scissors", [800, 800, 0, 0, 1000, 1000])
    ...
    async def _async_setup_and_run(self):
        ...
        try:
            await self._client.set_finger_positions(self._slave_id, [0, 0, 0, 0, 0, 0])
            await asyncio.sleep(1.0)
            while not self._shutdown_event.is_set():
                try:
                    # await self._client.set_finger_currents(self._slave_id, self._current)
                    await self._client.set_finger_positions(self._slave_id, self._pos_scissors)
                except Exception as e:
